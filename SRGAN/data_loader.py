...
class DataLoader():
    ...
    def load_data_tf(self, batch_size=1, is_testing=False):
        data_type = "train" if not is_testing else "test"
        ...
        for img_path in batch_images:
            ...
            img_hr = tf.image.decode_jpeg(image, channels=3)
            img_hr = tf.image.convert_image_dtype(img_hr, dtype=tf.float32)
            img_hr -= 0.5
            img_hr /= 0.5

            img_lr = tf.image.decode_jpeg(image, channels=3)
            img_lr = tf.image.convert_image_dtype(img_lr, dtype=tf.float32)
            img_lr -= 0.5
            img_lr /= 0.5
